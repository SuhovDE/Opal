...
--   TYPES4GROUPS (View)
--
CREATE OR REPLACE FORCE VIEW OPAL_FRA.TYPES4IN_G
...
select j."OBJ_NAME",j."CODE",j."VCODE",j."DESCR",j."GRP_ID",j."TYPE_ID",j."LVL",j."PARENT_ID",j."RN",j."GRP_TYPE",j."PRN",j."IS_LEAF",j."PREDEFINED",j."FID",j."ORDNO_IN_ROOT",j."DOM_ID",j."DIM_LEV_ID",j."DATE_FROM",j."DATE_TO",j."IS_GRP_PRIV",j."CR_USER",
   ...
   else coalesce((select max(0) from grp_tree t
        where parent_id=j.fid  and exists(select 1 from groups where id=t.id and coalesce(is_leaf,0)<>1)
         and not exists(select 1 from grp_tree where id=t.parent_id and case when parent_id<>0 then parent_id end is not null)
		 )
     , 1)
   end show_in_full
from(
...
from groups_tree g, types4groups tc
