...
--
CREATE OR REPLACE FORCE VIEW OPAL_FRA.SHOW_GROUPS4TYPES
...
BEQUEATH DEFINER
AS 
select OBJ_NAME,CODE,VCODE,concat(DESCR, case when date_from is not null then concat(concat(to_char(date_from, 'yyyy.mm.dd'),'-'), to_char(date_to, 'yyyy.mm.dd')) end) descr,
