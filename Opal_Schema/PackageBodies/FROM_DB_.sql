...
--   STANDARD (Package)
--
CREATE OR REPLACE PACKAGE BODY OPAL_FRA.From_Db_ AS
...
----------------------------------------------------------------------------------
 ...
 BEGIN --now doesn't work without CODE line in rs_...
  ...
  ELSE
   IF get_rs_suf THEN
    FOR c IN (SELECT * FROM TABLE(SELECT ATTR_INFO_TYPE FROM show_qrys_sel WHERE
               qry_id=qry_head.ID AND ordno=sel_list(i).ordno)
      ORDER BY DECODE(attr_type_code, Constants.code_code, 1, Constants.group_code, 2, 3)) LOOP
     if frst then
